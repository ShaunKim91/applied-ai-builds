...
from .routers import admin, auth, chat, documents, health, retrieval
...
for sub in ("uploaded_documents", "seed_corpus", "chroma"):
...
def _warm_cache() -> None:
    ...
    logger.info("Bootstrap: indexing seed corpus (Federalist Papers EN + Korean Wikipedia) ...")
    ok_corpus = _warm_step("dataset:seed_corpus", documents.index_seed_corpus)
    logger.info("Bootstrap: seed corpus step done (ok=%s).", ok_corpus)
    ...
    if state.bootstrap_step_errors:
        ...
        logger.info("Bootstrap: all models + seed corpus warm. Lucent is fully ready.")
