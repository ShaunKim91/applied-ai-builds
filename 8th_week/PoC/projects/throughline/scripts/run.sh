#!/usr/bin/env bash
# Start (or resume) an already-set-up Throughline instance. Fast — reuses
# the built image and the persisted data/model-cache volumes from setup.sh.
set -euo pipefail
...
if docker compose ps --status running --services 2>/dev/null | grep -qx app; then
  echo "Throughline is already running: http://localhost:${APP_PORT}"
  exit 0
fi
...
echo "Throughline: http://localhost:${APP_PORT}"
