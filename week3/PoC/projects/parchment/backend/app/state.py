...
bootstrap_step_errors is a per-step map (see debug/issue-06): each of the
6 startup steps (2 dataset downloads + 4 model loads) is isolated so a
transient failure in ONE step (e.g. a DNS blip fetching the UCI dataset)
can never take the others down with it — every step is attempted regardless
