"""The "smart layer" shared across all three extraction tabs: turns raw OCR
text into structured JSON fields, and writes PDF/HTML-table insight text.

Local Qwen2.5-0.5B-Instruct is the default, no-key-required path (same
model validated/reused in the Week1 and Week2 PoCs). OpenRouter's
qwen/qwen3-8b is a strictly opt-in, user-toggled upgrade — validated with a
real key this round, per the project brief. OpenAI/Anthropic/Gemini are
scaffolded (config fields exist in config.py) but NOT validated: calling
them raises a clear NotImplementedError instead of silently pretending to
