...
Parchment is a single-container full-stack application: a React (TypeScript) single-page app served as static files by a FastAPI backend, which hosts six AI models (five local, one optional cloud) behind a REST API, backed by SQLite (structured data) and Chroma (vector data, used for duplicate detection) — the same proven shape as the Week1/11 PoCs, applied to three technical pillars (computer vision/multimodal document reading, PDF parsing + summarization, HTML table scraping).
...
| Choice | Reasoning |
|---|---|
| **Same FastAPI + React template as the Week1/11 PoCs** | A proven, already-hardened architecture (auth, admin, Docker, readiness probing, isolated bootstrap steps) is reused deliberately rather than reinvented — see §5 for the specific hardening carried over. |
...
## 5. Hardening carried over from the Week1/11 PoCs (applied from day one here)
