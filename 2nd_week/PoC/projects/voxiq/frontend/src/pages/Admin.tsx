...
const KEY_FLAG_FIELDS = new Set(["openrouter_key_present", "e2b_key_present"]);
...
export default function Admin() {
  ...
  const [tab, setTab] = useState<"users" | "log" | "system">("system");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-edge text-sm">
        {(["system", "users", "log"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setTab(k)}
            className={`px-4 py-2 border-b-2 -mb-px ${
              tab === k ? "border-accent text-accent font-medium" : "border-transparent text-ink-secondary"
            }`}
          >
            {k === "system" ? t("admin.system") : k === "users" ? t("admin.users") : t("admin.auditLog")}
          </button>
        ))}
      </div>
      ...
      {tab === "system" && system && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="card p-5">
            <h3 className="text-sm font-medium text-ink-secondary mb-3">Record counts</h3>
          ...
          <div className="card p-5">
            <h3 className="text-sm font-medium text-ink-secondary mb-3">AI models loaded</h3>
            <dl className="text-sm space-y-1.5">
              {Object.entries(system.models).map(([k, v]) => (
                <div key={k} className="flex justify-between items-center">
                  ...
                  <dd>
                    ...
                    {KEY_FLAG_FIELDS.has(k) ? (v ? "key found" : "no key file") : v ? "loaded" : "not yet"}
                  </dd>
                </div>
              ))}
              ...
              <div className="flex justify-between">
                <dt className="text-ink-muted">sandbox provider</dt>
                <dd>{system.sandbox_provider}</dd>
              </div>
            </dl>
          </div>
          {system.disk && (
            <div className="card p-5 md:col-span-2">
              <h3 className="text-sm font-medium text-ink-secondary mb-3">Disk (data volume)</h3>
