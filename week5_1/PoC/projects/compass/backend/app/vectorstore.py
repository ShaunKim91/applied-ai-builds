"""Vector store wrapper — indexes past research reports (and their source
snippets) so the Archive page's semantic search actually works.
...
vector store or persistence at all** — every research run is
stateless/in-memory, so there is nothing to compare this against there;
the nearest real precedent is the Week4 "Lucent" PoC's `PersistentClient`
usage, reused verbatim here. Falls
