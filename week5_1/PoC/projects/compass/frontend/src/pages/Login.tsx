...
export default function Login() {
  ...
  const [email, setEmail] = useState("admin@compass.local");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center text-ink px-4">
      <div className="absolute top-4 right-4 flex gap-2">
        <button onClick={() => setLang(lang === "en" ? "ko" : "en")} className="card text-sm px-3 py-1.5 hover:bg-accent-soft">
          {t("common.language")}
        </button>
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card p-6">
          <div className="flex mb-6 rounded-lg border border-edge overflow-hidden text-sm">
            <button
          ...
          <form onSubmit={submit} className="space-y-4">
            {mode === "signup" && (
              <input
                className="w-full px-3 py-2 rounded-lg border border-edge bg-transparent text-ink text-sm"
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full py-2.5 rounded-lg bg-accent-gradient text-white text-sm font-medium disabled:opacity-60 shadow-card"
            >
