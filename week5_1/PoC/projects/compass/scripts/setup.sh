...
echo ""
echo "Waiting for the API to respond (all 3 local AI models download/warm up"
echo "in the background on first boot — this can take several minutes)..."
ATTEMPTS=0
...
echo ""
echo "✅ Compass is running at: http://localhost:${APP_PORT}"
echo "   Admin login: admin@compass.local / ChangeMe123!  (set ADMIN_PASSWORD in .env before real use)"
echo ""
echo "   Model warm-up may still be finishing in the background — watch progress with:"
echo "     docker compose logs -f"
echo "   Once 'Bootstrap: all models warm, web search reachable.' appears, run the full check with:"
