#!/usr/bin/env bash
# Start (or resume) an already-set-up Parchment instance. Fast — reuses the
...
if docker compose ps --status running --services 2>/dev/null | grep -qx app; then
  echo "Parchment is already running: http://localhost:${APP_PORT}"
  exit 0
fi
...
echo "Parchment: http://localhost:${APP_PORT}"
