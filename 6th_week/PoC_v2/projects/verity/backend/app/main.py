"""Verity — entrypoint: startup bootstrap (with a production insecure-
defaults guard, new this round), request correlation ID + latency
middleware, a global structured-error handler, and SPA static serving."""
import logging
import sys
import threading
import time
...
from .routers import admin, archive, auth, cat_events, health, radar, research, status
from .search import web_search
from .security import hash_password

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] verity: %(message)s")
logger = logging.getLogger("verity")

# --- Production insecure-defaults guard --------------------------------
# A real commercial-grade requirement none of the prior PoCs enforced: this
# app refuses to boot when ENFORCE_SECURE_DEFAULTS=true (a real production
# deployment's own opt-in) with a known-insecure secret key or admin
# password still in place, rather than silently running unsafely. Off by
# default so this demo boots immediately with its documented credentials —
# see config.py's docstring for why this isn't simply tied to app_env.
if settings.enforce_secure_defaults:
    insecure = []
    if settings.secret_key == "verity-dev-secret-change-in-production":
...
def _seed() -> None:
    ...
    try:
        ...
        if not org:
            org = models.Organization(name=settings.org_name, licensed_jurisdictions_json='["Ashford","Belmont Bay","Cedermoor","Dunraven","Elmsworth","Fairhaven"]')
            db.add(org)
            db.commit()
            db.refresh(org)
...
async def observability_middleware(request: Request, call_next):
    ...
    except Exception as exc:  # noqa: BLE001 — must classify + log every unhandled error
        db = SessionLocal()
        try:
            db.add(
                models.ErrorLog(
...
def _warm_cache() -> None:
    ...
    _warm_step("reranker", lambda: reranker.rerank("test", ["test document"]))
    logger.info("Bootstrap: loading local LLM (%s) ...", settings.local_llm_model)
    _warm_step("local_llm", llm.local_model_info)
    _warm_step("search:connectivity", web_search.connectivity_check)
    state.mark_bootstrap_complete()
    ready = state.readiness()
    if ready["all_warm"]:
        logger.info("Bootstrap: all models warm. Verity is fully ready.")
    else:
...
app.include_router(cat_events.router)
