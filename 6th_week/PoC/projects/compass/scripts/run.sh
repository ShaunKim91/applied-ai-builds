#!/usr/bin/env bash
# Start (or resume) an already-set-up Compass instance. Fast — reuses the
...
if docker compose ps --status running --services 2>/dev/null | grep -qx app; then
  echo "Compass is already running: http://localhost:${APP_PORT}"
  exit 0
fi
...
echo "Compass: http://localhost:${APP_PORT}"
