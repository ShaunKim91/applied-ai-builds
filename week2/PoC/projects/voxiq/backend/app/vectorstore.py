"""Vector store wrapper for the semantic catalog search feature.

Primary backend: Chroma (persistent, local, no external service — a
common, well-supported choice for this kind of local vector store). Falls
back automatically to a pure-Python cosine-similarity list if chromadb fails
to import/initialize, following the same "fallback pattern" already used
throughout this project series' own code, so the feature never hard-fails.
"""
import math
import threading
            ...
            if _collection is None:
                try:
                    ...
                    _collection = client.get_or_create_collection(
                        name="catalog", metadata={"hnsw:space": "cosine"}
                    )
                except Exception:
                    _collection = False
    return _collection
