...
- **Why chosen**: a well-suited small instruction-tuned model for a local ReAct agent's own
  reasoning brain — also the same small local LLM validated across the Week1-6 PoCs.
...
- **How Cradle uses it**: one best-effort final-answer synthesis call over the full ReAct trace —
  ...
  daily-budget-cap pattern Week6 Compass validated.
...
| **Audit trail** | The `audit_logs` SQL table — every AI inference call and tool-execution decision is recorded with who, what, how long, and success/failure, mirroring the pattern reused from the Week1-6 PoCs. This week's own build found and fixed a real gap in it: the `latency_ms` column was hardcoded to `0.0` everywhere, never actually measuring real call duration. See `debug/issue-06`. |
