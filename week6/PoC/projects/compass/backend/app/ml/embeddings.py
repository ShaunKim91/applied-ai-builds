"""Model 1 — bi-encoder embeddings for search-result reranking + archive search.

intfloat/multilingual-e5-small: the same embedding model validated in the
Week4 "Lucent" PoC (and, independently, confirmed as the same model a
typical baseline implementation of this pattern actually uses) — reused
here for its multilingual coverage (100+ languages sharing one embedding
space), which
Compass relies on for embedding both live web-search snippets (Korean or
English queries alike) and past research reports for archive search.
