...
> Week5_2 PoC · A Local ReAct Agent Console with Real Safety Guardrails
...
| Element | Week5_1 "Compass" | Week5_2 "Cradle" |
|---|---|---|
...
| A greedy regex over-matched a real small-model output (Week5_1) | Not directly applicable this week (no JSON-extraction step) — `react_loop.py`'s `truncate_generation()` uses an explicit stop-sequence scan, not a greedy regex, for the same reason |
...
real concurrent load, the same caveat Week5_1 Compass's own architecture.md raises for its budget
cap.
...
Notably lower than Week5_1 Compass — no licensed search API and no per-request managed web-search
