"""Cradle — FastAPI entrypoint.

Responsibilities on startup:
  1. Create SQL tables (SQLAlchemy metadata) if they don't exist yet.
  2. Seed the admin account + default guardrail/budget settings (idempotent).
  3. Kick off a *background* thread that loads the embedding model and the
     local agent LLM — each step isolated in its own try/except (the
     Week1-14_1 PoCs' `_warm_step()` pattern). /api/health responds
...
from .routers import admin, agent, approvals, auth, health, archive
...
def _seed_admin_and_settings() -> None:
    db = SessionLocal()
    try:
        ...
        if not db.get(models.GuardrailSetting, 1):
            db.add(
            ...
            logger.info("Seeded default guardrails: max_steps=%d cost_cap=%d", settings.default_max_steps, settings.default_cost_cap)
...
def _warm_cache() -> None:
    ...
    _warm_step("model:local_agent", react_loop.local_model_info)
    ...
    if state.bootstrap_step_errors:
        ...
        logger.info("Bootstrap: all models warm. Cradle is fully ready.")
    state.bootstrap_complete.set()


@app.on_event("startup")
def on_startup() -> None:
    _seed_admin_and_settings()
