...
Primary backend: Chroma (persistent, local, no external service). Falls
back automatically to a pure-Python cosine-similarity list if chromadb
fails to import/initialize, following the same "fallback pattern" used
elsewhere across this project series, so the feature never hard-fails.
"""
import math
import threading
