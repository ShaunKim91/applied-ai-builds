...
def system_status(db: Session = Depends(get_db), _admin: models.User = Depends(require_admin)):
    ...
    counts = {
        ...
        "catalog_items": db.query(func.count(models.CatalogItem.id)).scalar(),
        "generated_images": db.query(func.count(models.GeneratedImage.id)).scalar(),
        "forecast_runs": db.query(func.count(models.ForecastRun.id)).scalar(),
        "audit_logs": db.query(func.count(models.AuditLog.id)).scalar(),
    }
    models_status = {
        "vision_loaded": vision_ml._model is not None,
        "diffusion_loaded": diffusion_ml._pipe is not None,
        "embedding_loaded": emb_ml._model is not None,
