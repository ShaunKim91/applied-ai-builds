...
export default function Login() {
  ...
  const [email, setEmail] = useState("admin@commerceiq.local");
    ...
    <div className="min-h-screen flex items-center justify-center bg-page text-ink px-4">
      <div className="absolute top-4 right-4 flex gap-2">
        <button
          ...
          className="text-sm px-3 py-1.5 rounded-lg border border-edge hover:bg-accent-soft/60"
        >
          {t("common.language")}
        </button>
        <button onClick={toggle} className="text-sm px-3 py-1.5 rounded-lg border border-edge hover:bg-accent-soft/60">
          {theme === "dark" ? "☀️" : "🌙"}
        </button>
      </div>
        ...
        <div className="card p-6">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full py-2.5 rounded-lg bg-accent text-white text-sm font-medium disabled:opacity-60"
            >
