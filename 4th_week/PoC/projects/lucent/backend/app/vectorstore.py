"""Vector store wrapper for RAG chunk retrieval — the core index behind
every chat answer's citations.

Primary backend: Chroma, via `PersistentClient` (survives container
restarts — a real improvement over a naive setup that uses an in-memory
`chromadb.Client()` that resets every time the app restarts). Falls back
automatically to a pure-Python cosine-similarity list if chromadb fails to
import/initialize, following the same "fallback pattern" already used
throughout this project series, so the feature never hard-fails.
"""
import math
import threading
